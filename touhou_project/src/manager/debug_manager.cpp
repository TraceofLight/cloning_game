    ...
    if (debug_mode_on_) {
      ...
      switch (iter->shape) {
        ...
        case DEBUG_SHAPE::CROSS:
          MoveToEx(back_dc, static_cast<int>(iter->position.x() - iter->scale.x() / 2.f),
                   static_cast<int>(iter->position.y()), nullptr);
          LineTo(back_dc, static_cast<int>(iter->position.x() + iter->scale.x() / 2.f),
                 static_cast<int>(iter->position.y()));
          MoveToEx(back_dc, static_cast<int>(iter->position.x()),
                   static_cast<int>(iter->position.y() - iter->scale.y() / 2.f), nullptr);
